      ...
      <div style={{margin:'0 24px 16px',padding:16,borderRadius:24,...glass}}>
        ...
        <div className="cal-grid">
          {days.map((d,i)=>{const s=d.ds?getDay(d.ds):null;const canSet=corr&&d.ds;
            return(<div key={i} className={`cal-d${d.td?' today':''}${d.d===0?' dim':''}`} onClick={()=>{if(!canSet)return;if(corr==='start'){saveCycleData({...cycleData!,lastPeriodDate:d.ds});}if(corr==='end'){const start=new Date(cycleData.lastPeriodDate+'T00:00:00');const end=new Date(d.ds+'T00:00:00');const diff=Math.round((end.getTime()-start.getTime())/86400000)+1;if(diff>=2&&diff<=10)saveCycleData({...cycleData,periodLength:diff});}setCorr(null);}} style={canSet?{cursor:'pointer',borderRadius:12,background:corr==='start'?'rgba(244,152,183,.1)':'rgba(168,230,207,.1)'}:{}}>
